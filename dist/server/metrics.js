...
export function noteRun(success, durationMs) {
    totalRuns += 1;
    if (!success)
        totalFailures += 1;
      ...
      document.querySelectorAll('.navBtn').forEach(b=>b.addEventListener('click',()=>{
        ...
        // /metrics (Prometheus or HTML)
            ...
            (async () => {
                ...
                try {
                    const lastFull = await getState('incremental:lastFullRefreshTs');
                    if (lastFull) {
                ...
                if (lastSummary) {
                    for (const e of lastSummary.entities) {
                        ...
                        if (typeof e.fetched === 'number')
                            lines.push(`sync_entity_fetched_total{${lbl}} ${e.fetched}`);
                        if (typeof e.upserted === 'number')
                            lines.push(`sync_entity_upserted_total{${lbl}} ${e.upserted}`);
                        if (typeof e.processed === 'number')
                            lines.push(`sync_entity_processed_total{${lbl}} ${e.processed}`);
                        if (typeof e.pages === 'number')
                            lines.push(`sync_entity_pages_total{${lbl}} ${e.pages}`);
                        if (typeof e.total === 'number')
                            lines.push(`sync_entity_api_total{${lbl}} ${e.total}`);
                        if (typeof e.softDeleted === 'number')
                            lines.push(`sync_entity_soft_deleted_total{${lbl}} ${e.softDeleted}`);
                        if (typeof e.newMax === 'number')
                            lines.push(`sync_entity_newmax{${lbl}} ${e.newMax}`);
                        if (typeof e.lastMax === 'number')
                            lines.push(`sync_entity_lastmax{${lbl}} ${e.lastMax}`);
        ...
        // /summaries list
            ...
            (async () => {
                try {
                    ...
                    const rows = docs.map(d => { const started = d.start ? new Date(d.start).toLocaleString() : ''; const ended = d.end ? new Date(d.end).toLocaleString() : ''; return `<tr data-id="${d._id}"><td>${started}</td><td>${ended}</td><td>${human(d.durationMs || 0)}</td><td>${d.success ? '✔' : '✖'}</td><td>${d.error ? String(d.error).substring(0, 40) : ''}</td><td>${(d.entities || []).length}</td></tr>`; }).join('');
                    res.statusCode = 200;
                    res.setHeader('Content-Type', 'text/html; charset=utf-8');
        ...
        // /upserts
            ...
            if (sinceParam) {
                if (/^\d+$/.test(sinceParam)) {
                    const mins = parseInt(sinceParam, 10);
                    query.ts = { $gte: new Date(Date.now() - mins * 60000) };
                }
                else {
            ...
            (async () => {
                try {
                    ...
                    const formatVal = (v) => {
                        if (v === undefined)
                            return '<em style="color:#666">undefined</em>';
                        if (v === null)
                            return '<em style="color:#666">null</em>';
                        if (typeof v === 'string') {
                            const escStr = esc(v);
                            return escStr.length > 160 ? escStr.slice(0, 160) + '…' : escStr;
                        }
                        try {
                            const j = esc(JSON.stringify(v));
                    ...
                    const buildDiffTable = (changes) => {
                        const keys = Object.keys(changes || {});
                        if (!keys.length)
                            return '<div style="font-size:.65rem;color:#888">(no field-level changes captured)</div>';
...
function refresh(){var p=new URL(window.location.href);if(p.searchParams.get('runId'))return;var openIds=[].slice.call(document.querySelectorAll('tr.diff')).filter(function(r){return r.style.display!=='none';}).map(function(r){return r.id;});var jsonUrl=p.pathname+p.search+(p.search?'&':'?')+'format=json';fetch(jsonUrl).then(r=>r.json()).then(list=>{tableBody.innerHTML=buildRows(list);openIds.forEach(function(rowId){var row=document.getElementById(rowId);if(row){row.style.display='table-row';var id=rowId.replace(/^diff-/,'');var btn=document.querySelector('button.diff-btn[data-id="'+id+'"]');if(btn){btn.textContent='Hide';btn.classList.add('open');}}});}).catch(()=>{});}setInterval(refresh,15000);document.addEventListener('click',function(e){var t=e.target; if(!t) return; if(t.getAttribute && t.getAttribute('data-id') && t.classList.contains('diff-btn')){var id=t.getAttribute('data-id');var row=document.getElementById('diff-'+id); if(row){var open=row.style.display!=='none';row.style.display=open?'none':'table-row';t.textContent=open?'View':'Hide';t.classList.toggle('open',!open);} }});})();</script></body></html>`);
        ...
        // /timers (JSON only)
            ...
            (async () => {
                let { nextCronTs, nextFullRefreshTs, inProgress } = getLastSummary();
                ...
                if (cronEnabled && !nextCronTs) {
                    try {
                        ...
                        const it = parserMod.parseExpression(config.cron, { currentDate: new Date() });
                        nextCronTs = it.next().getTime();
                        setNextCron(nextCronTs);
                    }
                    catch (e) { }
                }
        ...
        // /trigger-sync (local only)
            ...
            if (inProgress) {
                res.statusCode = 409;
                return res.end('Sync already in progress');
            }
            runSync().then(() => { const { lastSummary } = getLastSummary(); if (lastSummary)
                noteRun(lastSummary.success, lastSummary.durationMs); }).catch(err => logger.error({ err }, 'Manual sync failed'));
            res.statusCode = 202;
            return res.end('Sync triggered');
        }
        else {
            res.statusCode = 404;
            res.end('Not found');
